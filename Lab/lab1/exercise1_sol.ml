...
let favourizer (arg:string)  = 
    arg ^ " is my favourite programming language.";;
...
let why1 = (1.0 = 1.0) 
let why2 = (1.0 == 1.0)
let because = "= compares the two values, == compares the two variables or objects address.";;
...
let rec square (x:float) (g:float) = 
    if closeEnough (x /. g) g then
        g
    else
        square x ( (g +. (x /. g)) /. 2.0 )
