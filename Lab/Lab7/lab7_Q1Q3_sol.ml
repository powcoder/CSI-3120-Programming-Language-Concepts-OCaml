...
(* A version with printing to help follow the execution flow *)
let rec f1' (x:int) =
  match x with
  | 0 -> (print_string "0"; 1)
...
Call (f1 1)
raise Odd
pop activation record for (f1 1) without returning control to the function
pop activation record for (f1 3) without returning control to the function
in activation record for (f1 5), handle the Odd exception
return the value -5 from the call (f1 5)
return the value -5 from the call (f1 7)
return the value -5 from the call (f1 9)
return the value -5 from the call (f1 11)
 *)
